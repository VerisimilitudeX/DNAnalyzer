...
public class SequenceAnalysisService {
  ...
  private static final Map<String, String> GENETIC_CODE =
      Map.ofEntries(
          ...
          Map.entry("GGA", "G"),
          Map.entry("GGG", "G"));
  private static final List<String> DEFAULT_ANALYSIS_OPTIONS =
      List.of(
          "sequence-length",
          "gc-content",
          "start-codons",
  ...
  public BasePairResponse analyzeBasePairs(SequenceRequest request) {
    ...
    double gcContent =
        toPercentage(counts.getOrDefault('G', 0L) + counts.getOrDefault('C', 0L), length);
    ...
    if (startAminoAcid != null && !startAminoAcid.isBlank()) {
      ...
      if (aa != 'M') {
        throw new IllegalArgumentException(
            "Only methionine (M) start codon is supported at this time");
      }
    }
          ...
          while (j <= normalized.length() - 3) {
            ...
            if (STOP_CODONS.contains(nextCodon)) {
              if (proteinSequence.length() >= minLength) {
                proteins.add(
                    new Protein(
                        proteinStart, j + 2, proteinSequence.length(), proteinSequence.toString()));
              }
              i = j + 3;
              break;
            }
  ...
  public ManipulationResponse manipulate(ManipulationRequest request) {
    ...
    String reverseComplement =
        (reverse && complement) ? ReverseComplementer.reverseComplement(normalized) : null;

    return new ManipulationResponse(normalized, reversed, complemented, reverseComplement);
  }

  public AnalysisResponse analyzeSequence(
      String fileName, String format, String sequence, List<String> requestedOptions) {
    String normalized = normalizeSequence(sequence);
    SequenceAnalyzer analyzer = new SequenceAnalyzer(normalized);
    ...
    ReadingFrameResponse readingFrames =
        analyzeReadingFrames(new ReadingFrameRequest(normalized, 300));
    ...
    return new AnalysisResponse(
        fileName, format, summary, basePairs, codons, readingFrames, proteins, options);
  }

  public FileParseResponse buildFileParseResponse(
      String originalName, String header, String sequence, String format) {
    String normalized = normalizeSequence(sequence);
  ...
  private ReadingFrame analyzeFrame(
      String sequence, int offset, int frameNumber, String direction, int minLength) {
