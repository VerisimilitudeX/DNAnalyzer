...
public final class PolygenicRiskCalculator {
  ...
  public PolygenicRiskResult calculate(GenotypeData genotypeData, RiskWeightTable weightTable) {
    ...
    for (RiskWeightEntry entry : weightTable.entries()) {
      ...
      if (record == null) {
        missing++;
        contributions.add(
            VariantContribution.missing(
                entry.rsid(), entry.riskAllele(), entry.weight(), "No genotype call"));
        continue;
      }
      ...
      if (dosage < 0) {
        ...
        contributions.add(
            VariantContribution.missing(
                ...
                entry.riskAllele(),
                entry.weight(),
                "Uncallable genotype: " + record.genotype()));
        continue;
      }
    ...
    double coverage =
        weightTable.entries().isEmpty() ? 0.0 : matched / (double) weightTable.entries().size();
  ...
  public record VariantContribution(
    ...
    private static VariantContribution matched(
        ...
        double weight,
        double contribution) {
      return new VariantContribution(
          rsid, genotype, riskAllele, dosage, weight, contribution, true, "");
    }

    private static VariantContribution missing(
        String rsid, String riskAllele, double weight, String note) {
