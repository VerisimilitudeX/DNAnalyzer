  ...
  private void loadPlugins() {
    ...
    try (DirectoryStream<Path> stream = Files.newDirectoryStream(pluginDir, "*.jar")) {
      for (Path jar : stream) {
        ...
        ServiceLoader<DNAnalyzerPlugin> serviceLoader =
            ServiceLoader.load(DNAnalyzerPlugin.class, loader);
        for (DNAnalyzerPlugin plugin : serviceLoader) {
          try {
            ...
            System.err.println(
                "Failed to init plugin from " + jar.getFileName() + ": " + e.getMessage());
          }
        }
      }
    } catch (IOException e) {
  ...
  public void runPlugins(DNATools dna, PrintStream out) {
    for (DNAnalyzerPlugin plugin : plugins) {
      try {
        ...
        System.err.println("Plugin " + plugin.getClass().getName() + " failed: " + e.getMessage());
      }
    }
  }
}
