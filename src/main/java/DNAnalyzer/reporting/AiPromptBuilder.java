...
final class AiPromptBuilder {
  ...
  private static String buildUserMessage(AiReportContext context) {
    ...
    builder
        .append("Create two narratives about the DNA analysis below.\n")
        .append(
            "- researcher_report: technical tone for genomics researchers, reference statistics,"
                + " note risks.\n")
        .append(
            "- layperson_report: plain language, short paragraphs, include actionable guidance.\n")
    ...
    builder.append(
        String.format(Locale.ROOT, "  gc_content_percent: %.2f%n", context.gcContent() * 100));
    builder
        .append("  nucleotide_counts: ")
        .append(formatCounts(context.nucleotideCounts()))
        .append('\n');
    ...
    builder.append(
        String.format(Locale.ROOT, "  mutations_generated: %d%n", context.mutationSequences()));
    builder.append(
        String.format(Locale.ROOT, "  mutation_edits_per_sequence: %d%n", context.mutationEdits()));
    builder.append(
        String.format(
            Locale.ROOT,
            "  reverse_complement_generated: %s%n",
