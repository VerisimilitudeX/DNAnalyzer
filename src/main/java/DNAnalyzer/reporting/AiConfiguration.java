...
final class AiConfiguration {
  ...
  static AiConfiguration resolve() {
    String provider =
        Optional.ofNullable(System.getenv("AI_PROVIDER"))
    ...
    String model =
        firstNonBlank(
            System.getenv("OPENAI_MODEL"), System.getProperty("openai.model"), "gpt-3.5-turbo");

    double temperature =
        parseDouble(
            firstNonBlank(
                System.getenv("OPENAI_TEMPERATURE"), System.getProperty("openai.temperature")),
            0.2);
    ...
    int maxTokens =
        (int)
            parseDouble(
                firstNonBlank(
                    System.getenv("OPENAI_MAX_TOKENS"), System.getProperty("openai.maxTokens")),
                700);
  ...
  private static String readPropertyFromResource(String key) {
    try (InputStream stream =
        AiConfiguration.class.getClassLoader().getResourceAsStream(PROPERTIES_RESOURCE)) {
      if (stream == null) {
        return null;
      }
