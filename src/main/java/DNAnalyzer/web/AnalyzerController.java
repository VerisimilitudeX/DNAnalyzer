...
public class AnalyzerController {
  ...
  private static final Set<String> VALID_AMINO_ACIDS =
      new HashSet<>(
          Arrays.asList(
              "met", "ser", "thr", "val", "ala", "arg", "asn", "asp", "cys", "gln", "glu", "gly",
              "his", "ile", "leu", "lys", "phe", "pro", "trp", "tyr"));
  ...
  public ResponseEntity<String> analyzeDNA(
    ...
    try {
      ...
      if (!VALID_AMINO_ACIDS.contains(aminoLower)) {
        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid amino acid specified");
      }
      ...
      if (dnaFile != null && !dnaFile.isEmpty()) {
        ...
        if (dnaFile.getSize() > MAX_FILE_SIZE) {
          throw new ResponseStatusException(
              HttpStatus.BAD_REQUEST, "DNA file size exceeds maximum limit of 10MB");
        }
        ...
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "No DNA input provided. Please provide either a file or URL");
      }
      ...
      if (proteinFile != null && !proteinFile.isEmpty()) {
        if (proteinFile.getSize() > MAX_FILE_SIZE) {
          throw new ResponseStatusException(
              HttpStatus.BAD_REQUEST, "Protein file size exceeds maximum limit of 10MB");
        }
        proteinPath =
            Files.createTempFile("protein", getFileExtension(proteinFile.getOriginalFilename()));
      ...
      try {
        ...
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Invalid DNA sequence: " + e.getMessage());
      }
  ...
  public ResponseEntity<String> analyzeGenetic(
    ...
    if (geneticFile.getSize() > MAX_FILE_SIZE) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Genetic file size exceeds maximum limit of 10MB");
    }
    ...
    try {
      ...
      Path geneticPath =
          Files.createTempFile("genetic", getFileExtension(geneticFile.getOriginalFilename()));
      ...
      if (fileName.contains("23andme")) {
        ...
        throw new IllegalArgumentException(
            "Unsupported genetic data format. Please use 23andMe or AncestryDNA files.");
      }
  ...
  private Path downloadFile(String urlString, String prefix) throws IOException {
    ...
    if (!extension.equalsIgnoreCase(".fa") && !extension.equalsIgnoreCase(".fastq")) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Invalid file format. URL must point to a .fa or .fastq file");
    }
    ...
    if (contentLength > MAX_FILE_SIZE) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "File size exceeds maximum limit of 10MB");
    }
