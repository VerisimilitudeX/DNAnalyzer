...
class SequenceAnalysisServiceTest {
  ...
  void analyzeReadingFramesDetectsGenes() {
    ReadingFrameResponse response =
        service.analyzeReadingFrames(new ReadingFrameRequest("ATGAAATAG", 0));
  ...
  void findProteinsReturnsLongestUpToLimit() {
    ProteinResponse response =
        service.findProteins(new ProteinRequest("ATGAAATAGATGCCCCCTAA", "M", 2));
  ...
  void manipulateSequenceSupportsReverseAndComplement() {
    ManipulationResponse response = service.manipulate(new ManipulationRequest("ATGC", true, true));
  ...
  void analyzeSequenceProducesSummaryAndRespectsOptions() {
    AnalysisResponse response =
        service.analyzeSequence(
            "sample.fasta", "FASTA", "ATGAAATAG", List.of("sequence-length", "gc-content"));
