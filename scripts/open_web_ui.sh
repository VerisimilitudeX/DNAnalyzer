...
UI_FILE="$REPO_ROOT/web/dashboard/dashboard.html"
BACKEND_LOG_DIR="$REPO_ROOT/build"
BACKEND_LOG_FILE="$BACKEND_LOG_DIR/web-ui-backend.log"
BOOT_JAR="$REPO_ROOT/build/libs/DNAnalyzer-1.2.1-boot.jar"
...
"$PYTHON_BIN" -m http.server "$UI_PORT" --directory "$REPO_ROOT/web" \
  >"$UI_LOG_FILE" 2>&1 &
UI_PID=$!
